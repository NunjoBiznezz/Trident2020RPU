...
//    if ((CurrentTime-LastSolTestTime)>300) RPU_PlaySB100(128);
   ...
   if (savedScoreStartByte) {
      ...
      if (resetBeingHeld && (CurrentTime >= NextSpeedyValueChange)) {
         ...
         RPU_SetDisplay(0, SavedValue, true);
         if (NumSpeedyChanges < 6) {
            NextSpeedyValueChange = CurrentTime + 400;
         } else if (NumSpeedyChanges < 50) {
            NextSpeedyValueChange = CurrentTime + 50;
         } else {
            NextSpeedyValueChange = CurrentTime + 10;
         }
         NumSpeedyChanges += 1;
      }
